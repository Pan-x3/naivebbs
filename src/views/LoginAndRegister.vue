<template>
  <div>
    <Dialog :Show="dialogConfig.show" :title="dialogConfig.title" :buttons="dialogConfig.buttons" width="360px"
      :showCancel="false" @close="closeDialog">
      <el-form :model="formData" :rules="rules" ref="formDataRef" class="login-register">
        <!--input输入-->
        <el-form-item prop="email" required>
          <el-input size="large" clearable placeholder="请输入邮箱" v-model="formData.email" maxLength="100">
            <template #prefix>
              <span class="iconfont icon-account"></span> </template></el-input>
        </el-form-item>
        <!-- 登录密码 -->
        <el-form-item prop="password" required v-if="opType === 1">
          <el-input size="large" placeholder="请输入密码" v-model="formData.password" show-password>
            <template #prefix>
        ...
        <div v-if="opType === 0 || opType === 2">
          <el-form-item prop="emailCode" style="margin-bottom: 0;" required>
            <div class="send-email-panel">
              <el-input size="large" clearable placeholder="请输入邮箱验证码" v-model="formData.emailCode">
                <template #prefix>
                  <span class="iconfont icon-checkcode"></span> </template></el-input>
              <el-button type="primary" size="large" class="send-mail-btn" @click="getEmailCode">获取验证码</el-button>
            </div>
          ...
          <el-form-item prop="registerPassword" required>
            <el-input size="large" placeholder="请输入密码" v-model="formData.registerPassword" show-password>
              <template #prefix>
                <span class="iconfont icon-password"></span>
              </template>
            </el-input>
          ...
          <el-form-item prop="reRegisterPassword" required>
            <el-input size="large" placeholder="请再次输入密码" v-model="formData.reRegisterPassword" show-password>
              <template #prefix>
                <span class="iconfont icon-password"></span> </template></el-input>
          </el-form-item>
        </div>
        ...
        <el-form-item>
          ...
          <div class="no-account">
            ...
            <a href="javascript:void(0)" class="a-link" @click="showPanel(0)" v-if="opType === 1">注册</a>
            <a href="javascript:void(0)" class="a-link" @click="showPanel(1)" v-if="opType === 0">已有帐号</a>
            <a href="javascript:void(0)" class="a-link" @click="showPanel(1)" v-if="opType === 2">去登录</a>
          </div>
          </div>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" class="op-btn" @click="doSubmit"><span v-if="opType === 0">注册</span><span
              v-if="opType === 1">登录</span><span v-if="opType === 2">重置密码</span></el-button>
        </el-form-item>
      </el-form>
      <!-- 发送邮箱验证码 -->
      ...
      <el-form :model="formData4SendMailCode" :rules="rules" ref="formData4SendMailCodeRef" label-width="80px">
        ...
        <el-form-item label="验证码" prop="checkCode">
          <div :style="{ display: 'flex' }">
            <el-input size="large" clearable placeholder="请输入验证码" v-model="formData4SendMailCode.checkCode">
              <template #prefix>
